...
class Point {
  ...
  add (o) {
    return new Point({x: this.x + o.x, y: this.y + o.y})
  }
  sub (o) {
    return new Point({x: this.x - o.x, y: this.y - o.y})
  }
...
class YtFaceShape {
  constructor (shape) {
    ...
    for (i = 0; i < this.leftEyebrow.length; i ++) {
      this.leftEyebrow[i] = new Point(shape.points[idx])
      idx += 1
    }
    for (i = 0; i < this.rightEyebrow.length; i ++) {
      this.rightEyebrow[i] = new Point(shape.points[idx])
      idx += 1
    }
    for (i = 0; i < this.leftEye.length; i ++) {
      this.leftEye[i] = new Point(shape.points[idx])
      idx += 1
    }
    for (i = 0; i < this.rightEye.length; i ++) {
      this.rightEye[i] = new Point(shape.points[idx])
      idx += 1
    }
    for (i = 0; i < this.nose.length; i ++) {
      this.nose[i] = new Point(shape.points[idx])
      idx += 1
    }
    for (i = 0; i < this.mouth.length; i ++) {
      this.mouth[i] = new Point(shape.points[idx])
      idx += 1
    }
    for (i = 0; i < this.faceProfile.length; i ++) {
      this.faceProfile[i] = new Point(shape.points[idx])
      idx += 1
    }
    for (i = 0; i < this.pupil.length; i ++) {
      this.pupil[i] = new Point(shape.points[idx])
      idx += 1
    }
  }
}
